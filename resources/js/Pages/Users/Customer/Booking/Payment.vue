...
const groupedPaymentMethods = computed(() => {
    return Object.entries(props.paymentAccounts).map(([type, accounts]) => ({
        type: type,
        name: type === 'gcash' ? 'GCash' :
              type === 'paymaya' ? 'PayMaya' :
              'Bank Account',
        icon: type === 'gcash' ? 'fa-wallet text-blue-500' :
              type === 'paymaya' ? 'fa-mobile-screen-button text-purple-500' :
...
watch(() => props.availService, (newAvailService) => {
    if (newAvailService) {
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                ...
                <div class="mb-6 overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center gap-x-4">
                            <img
                                :src="transactionData.service_provider.profile_photo"
                ...
                <div class="overflow-hidden bg-white shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        ...
                        <div class="p-4 mb-6 bg-gray-50 rounded-lg">
                            <h3 class="mb-2 text-lg font-medium">Transaction Details</h3>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                    ...
                                    <p class="text-xl font-bold text-primary">₱{{ form.amount.toLocaleString() }}</p>
                                </div>
                                <div>
                                    <p class="text-gray-600">Provider</p>
                                    <p class="font-medium">{{ service.user?.name }}</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <form @submit.prevent="submit" class="space-y-6">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div v-for="type in transactionTypes"
                                        :key="type.value"
                                    ...
                                    >
                                        <div class="flex items-start gap-x-3">
                                            <div :class="[
                                                'mt-1 p-2 rounded-lg',
                                                form.transaction_type === type.value ? 'bg-primary/10' : 'bg-gray-100'
                                            ]">
                                                <i :class="[type.icon, 'text-lg', form.transaction_type === type.value ? 'text-primary' : 'text-gray-600']"></i>
                                            </div>
                                            <div>
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <div class="space-y-2">
                                        ...
                                        <div class="flex flex-col text-sm text-gray-600">
                                            <span>Total Price: ₱{{ availService.total_price.toLocaleString() }}</span>
                                            <span v-if="form.transaction_type === 'deposit'"
                                                :class="{'text-primary': remainingBalance > 0}">
                                                Remaining Balance: ₱{{ remainingBalance.toLocaleString() }}
                                            </span>
                                            <span v-if="form.transaction_type === 'deposit'" class="text-xs">
                                                Minimum Deposit Required: ₱{{ minimumPayment.toLocaleString() }} (30%)
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div class="p-4 bg-gray-50 rounded-lg">
                                        ...
                                        <div class="space-y-2 text-sm">
                                            <div class="flex justify-between">
                                                ...
                                                <span>₱{{ availService.total_price.toLocaleString() }}</span>
                                            </div>
                                            <div class="flex justify-between text-primary font-medium">
                                                <span>{{ form.transaction_type === 'deposit' ? 'Deposit Amount:' : 'Payment Amount:' }}</span>
                                                <span>₱{{ form.amount.toLocaleString() }}</span>
                                            </div>
                                            <div v-if="form.transaction_type === 'deposit'"
                            ...
                            <div class="space-y-6">
                                <h3 class="text-lg font-medium flex items-center gap-x-2">
                                    <i class="fa-solid fa-credit-card text-primary"></i>
                                    Select Payment Method
                                </h3>

                                <!-- Empty State Message -->
                                <div v-if="!Object.keys(groupedPaymentMethods).length"
                                    class="p-8 text-center bg-gray-50 rounded-lg border border-dashed border-gray-300">
                                    <div class="flex flex-col items-center gap-y-3">
                                        <i class="fa-solid fa-wallet text-4xl text-gray-400"></i>
                                        <div class="space-y-1">
                                            <h4 class="font-medium text-gray-900">No Payment Methods Available</h4>
                                            <p class="text-sm text-gray-600">
                                                The service provider hasn't added any payment methods yet.
                                            </p>
                                            <p class="text-sm text-gray-600">
                                                Please contact the provider to set up payment methods.
                                            </p>
                                        </div>
                                        <!-- Message Provider Button -->
                                        <Link
                                            :href="route('messages.index', { participant_id: service.user.id })"
                                            class="mt-2 inline-flex items-center px-4 py-2 text-sm font-medium text-primary bg-primary/10 rounded-lg hover:bg-primary/20 transition-colors"
                                        >
                                ...
                                <template v-else>
                                    <div v-for="group in groupedPaymentMethods" :key="group.type" class="space-y-4">
                                        <h3 class="text-lg font-medium flex items-center gap-x-2">
                                        ...
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            <div v-for="account in group.accounts"
                                                :key="account.id"
                                                @click="selectedAccount = account; form.payment_account_id = account.id"
                                                :class="[
                                            ...
                                            >
                                                <!-- Selected Indicator -->
                                                <div v-if="selectedAccount?.id === account.id"
                                                    class="absolute -top-2 -right-2 w-6 h-6 bg-primary text-white rounded-full flex items-center justify-center">
                            ...
                            <div>
                                ...
                                <TextInput
                                    ...
                                    required
                                />
                                <p class="mt-1 text-sm text-gray-500">Enter the reference number from your payment transaction</p>
                            ...
                            <div>
                                <InputLabel for="attachments" value="Proof of Payment" />
                                <input
                                    type="file"
                                    id="attachments"
                                    ...
                                    required
                                />
                                <p class="mt-1 text-sm text-gray-500">Upload screenshots or PDF of your payment receipt (Max 5MB per file)</p>
                            ...
                            <div class="flex justify-between items-center pt-6 border-t">
                                ...
                                >
                                    ...
                                    {{ form.processing ? 'Processing...' : 'Complete Payment' }}
                                </PrimaryButton>
                            </div>
                        </form>
                    </div>
